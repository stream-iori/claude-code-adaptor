...
use crate::models::qwen3::{QwenRequest, QwenInput, QwenMessage, QwenRole, QwenParameters, ToolDefinition as QwenToolDefinition, FunctionDefinition as QwenFunctionDefinition, ToolChoice as QwenToolChoice, FunctionChoice as QwenFunctionChoice};
...
impl RequestAdapter for ClaudeToQwenAdapter {
    type From = ClaudeRequest;
    type To = QwenRequest;
    
    fn adapt(&self, claude_request: Self::From) -> Result<Self::To> {
        ...
        let tools = claude_request.tools.map(|claude_tools| {
            claude_tools.into_iter().map(|tool| {
                QwenToolDefinition {
        ...
        let tool_choice = claude_request.tool_choice.map(|choice| {
            QwenToolChoice {
                choice_type: choice.choice_type,
                function: choice.function.map(|func| QwenFunctionChoice {
                    name: func.name,
                }),
            }
        });
